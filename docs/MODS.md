...
| Immersive Engineering | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/immersive-engineering) | 10.2.0-183 (1.20.1 Forge) | Added 2026-09-08 (Roadmap Phase 3) — the power generator (Diesel Generator et al.) for the Tier 3 chain, and separately the source of Tier 3's real Tesla Coil (`immersiveengineering:tesla_coil`, decompiled and picked over both the nonexistent "Create's own Tesla Coil" the roadmap dispatch named and Create: Crafts & Additions' real-but-smaller version — see FEATURES.md for the full mechanics comparison). Marked "Final release for 1.20.1" by the author. Real footprint: a full standalone tech mod (own ore processing, engineering workbench, multiblock machines) — the biggest single addition to this pack besides full Create | Confirmed via the downloaded jar's own `mods.toml`: only mandatory deps are `forge >=47.3.0` (this pack runs 47.4.10, satisfied) and `minecraft 1.20.1`; `jei` optional, already installed | testing |
...
| Advanced Tower Defense | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/advanced-tower-defense) | 3.7 (1.20.1 Forge) | Added 2026-09-08 (Track C) — Musket Sentry + Anvil Launcher, the real Tier 2 turret content; promoted to the Tier 2 gate outright on 2026-09-09 once Medieval Defense Turrets was cut (see below). Real modId `advanced_tower_defense_mod` (a full MCreator "tech tree" with 150+ turret variants; this pack only unlocks the two Tier-0 turret heads) | Jar hash-verified against packwiz's own resolved hash before any decompiling. Never previously listed in this table despite being installed since Track C — added now while touching related recipes | testing |
