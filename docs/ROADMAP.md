...
- Forge version pinned in `mod/gradle.properties` and `pack/pack.toml` — bump both together when updating.
- One pack, curated mods + glue — not a from-scratch content mod. Glue for
  ...
  first; the Java mod in `mod/` is a fallback for anything KubeJS can't do.
...
- Theme: tower defense — inspired by Diseased Craft, but waves are meant
  ...
  **horde-density** approach: no custom AI, mobs behave normally (track by
  ...
  step, deferred because it requires custom Java AI in `mod/` rather than
  just picking mods + KubeJS glue — bigger bet, revisit once the
  horde-density version is proven fun.

## Mod list
- See [docs/MODS.md](MODS.md) — tracked per-mod as they're picked.

## Custom mod (`mod/`, fallback only)
- [ ] Rename `examplemod` if/when it's actually needed for something KubeJS can't do.
